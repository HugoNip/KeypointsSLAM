...
void find_feature_matches(const cv::Mat &img_1, const cv::Mat &img_2,
    ...
    // find the max and min distance for each descriptor
...
void pose_estimation_2d2d(std::vector<cv::KeyPoint> keypoints_1,
    ...
    cv::Mat F_matrix = cv::findFundamentalMat(points1, points2, cv::FM_8POINT);
    std::cout << "Fundamental matrix is " << std::endl << F_matrix << std::endl;

    // compute Essential matrix
    cv::Point2d principal_point(325.1, 249.7); // cx, xy
    double focal_length = 521;
    cv::Mat E_matrix = cv::findEssentialMat(points1, points2, focal_length, principal_point);
    std::cout << "Essential matrix is " << std::endl << E_matrix << std::endl;

    // compute Homography matrix
    cv::Mat H_matrix = cv::findHomography(points1, points2, cv::RANSAC, 3);
    std::cout << "homography matrix is " << std::endl << H_matrix << std::endl;

    // recover R, t from E-matrix
